...
addOptional(p, 'pinvAb', defaultpinvAb, checkVector);
...
if ~isempty(restart)
    ...
    last_idx = 0;
    for k = 1:maxit
        [x0, exitflag, resvec1, Aresvec1, xresvec1] = inner_gmres(A, b, tol, restart, x0, criteria,pinvAb);

        len = length(resvec1);
        resvec(last_idx+1:len+last_idx) = resvec1;
        Aresvec(last_idx+1:len+last_idx) = Aresvec1;
        xresvec(last_idx+1:len+last_idx) = xresvec1;
        ...
        if exitflag == 0
            ...
            xresvec = xresvec(1:last_idx);
            break;
        end
    end
else
    [x, exitflag, resvec, Aresvec, xresvec] = inner_gmres(A, b, tol, maxit, x0, criteria,pinvAb);
end
...
function [x, exitflag, resvec, Aresvec,xresvec] = inner_gmres(A, b, tol, maxit, x0, criteria, pinvAb)
m = size(A, 1);
...
exitflag = 1; resvec = gc; Aresvec = gc; xresvec=gc;
for j = 1:maxit
    v = A * v1;
